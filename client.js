document.addEventListener('DOMContentLoaded', function(){
    ...
    originalVideo.addEventListener('play', function(){
        ...
        gaussianBlurDraw(originalVideo,context,backcontext,cw,ch);

        /*      
        backcontext.drawImage(originalVideo,0,0,cw,ch);
        var pixels = backcontext.getImageData(0,0,cw,ch);
        var idata = Filters.gaussianBlur(pixels,5);
        //var idata = Filters.grayscale(pixels,0);
...
function gaussianBlurDraw(v,c,bc,w,h) {
    if(v.paused || v.ended) return false;
    // First, draw it into the backing canvas
    bc.drawImage(v,0,0,w,h);
    // Grab the pixel data from the backing canvas
    var pixels = bc.getImageData(0,0,w,h);
    var idata = Filters.gaussianBlur(pixels,17);
    // Draw the pixels onto the visible canvas
    c.putImageData(idata,0,0);
    // Start over!
    //setTimeout(function(){gaussianBlurDraw(v,c,bc,w,h); }, 0);
    setTimeout(gaussianBlurDraw,50,v,c,bc,w,h);
}
...
/////////////////////////////////////////////////
/*
    Nguồn sử dụng thuật toán Gaussian Blur
